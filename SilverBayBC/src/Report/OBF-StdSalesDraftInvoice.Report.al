...
// This is based on report 1303 "Standard Sales - Draft Invoice"
report 50003 "OBF-Std. Sales-Draft Invoice"
{
    Caption = 'Draft Invoice';
    DefaultRenderingLayout = "OBFStdSalesDraftInvoice.rdlc";
    ...
    dataset
    {
        dataitem(Header; "Sales Header")
        {
            ...
            column(HideOffInvoiceLabel;HideOffInvoiceLabel)
            {
            }
            column(OBF_Header_Summary_Line_1;"OBF-Header Summary Line 1")
            {
            }
            // column(OBF_Header_Summary_Line_2;"OBF-Header Summary Line 2")
            // {
            // }
            ...
            column(CreationDate;Header.SystemCreatedAt)
            {
            }

            // https://odydev.visualstudio.com/ThePlan/_workitems/edit/1826 - Sales Invoice Enhancements
            column(ShowAdditionalAddresses;ShowAdditionalAddresses)
            {
            }
            ...
            dataitem(Line; "Sales Line")
            {
                ...
                column(MSCText;MSC_Text)
                {
                }
                column(RFMText;RFM_Text)
                {
                }
                
                column(OffInvoiceRebateAmountLine_Line; OffInvoicePerLbFormatted)
                {
                }
                ...
                begin
                    ...
                    SalesSetup.Get();
                    if not SalesSetup."OBF-Disable Rebates" then begin
                        Line.CalcFields("OBF-Off Invoice Rebate Amount");
                        Line."Line Amount" -= "OBF-Off Invoice Rebate Amount";
                        Line.Amount -= "OBF-Off Invoice Rebate Amount";
                        Line."Amount Including VAT" -= "OBF-Off Invoice Rebate Amount";

                        Line."OBF-Line Net Weight" := 0;
                        IF Type = Type::Item THEN begin
                            Item.Get("No.");
                            LineWeight := Item."Net Weight" * "Quantity (Base)";
                            if Item."OBF-Exclude from Weight Calc." then begin
                                "Qty. to Ship" := 0;
                                "Net Weight" := 0;
                            "Gross Weight" := 0;
                            LineWeight := 0;
                        ...
                        Line."OBF-Line Net Weight" := "Net Weight" * "Quantity (Base)";
                        OffInvoicePerLbFormatted := RebateManagement.CalculateOffInvoicePerLb(Line."OBF-Off Invoice Rebate Amount", Line."OBF-Line Net Weight");
                    end;
                    ...
                    MSC_Text := '';
                    RFM_Text := '';
                    if Line."OBF-MSC Certification" then
                        MSC_Text := 'X';
                    if Line."OBF-RFM Certification" then
                        RFM_Text := 'X';
            ...
            // https://odydev.visualstudio.com/ThePlan/_workitems/edit/1675 - Page and Report Cleanup
            ...
            begin
                ...
                // https://odydev.visualstudio.com/ThePlan/_workitems/edit/1675 - Page and Report Cleanup
                CurrReport.Language := CU_Language.GetLanguageIdOrDefault("Language Code");
                CurrReport.FormatRegion := CU_Language.GetFormatRegionOrDefault("Format Region");
                ...
                RebateEntry.SetRange(RebateEntry."Source Type",Header."Document Type");
                RebateEntry.SetRange(RebateEntry."Source No.",Header."No.");
                RebateEntry.SetRange(RebateEntry."Rebate Type",RebateEntry."Rebate Type"::"Off-Invoice");
                HideOffInvoiceLabel := RebateEntry.IsEmpty;
            end;
        ...
        layout
        {
            area(content)
            {
                group(Options)
                {
                    ...
                    // https://odydev.visualstudio.com/ThePlan/_workitems/edit/1826 - Sales Invoice Enhancements
                    field(ShowAdditionalAddresses; ShowAdditionalAddresses)
                    {
                        ApplicationArea = All;
                        Caption = 'Show Additional Addresses';
                    }
    ...
    rendering
    {
        layout("OBFStdSalesDraftInvoice.rdlc")
        {
            Type = RDLC;
            LayoutFile = './src/ReportLayouts/OBFStdSalesDraftInvoice.rdlc';
            Caption = 'Silver Bay Sales Draft Invoice (RDLC)';
            Summary = 'The Silver Bay Sales Draft Invoice (RDLC) provides a detailed layout.';
        }
        // layout("OBFStdSalesDraftInvoice.docx")
        // {
        //     Type = Word;
        //     LayoutFile = './Sales/Document/OBFStdSalesDraftInvoice.docx';
    ...
    var
        ...
        LineWeight: Decimal;
        HideOffInvoiceLabel: Boolean;
        OffInvoicePerLb: Decimal;
        OffInvoicePerLbFormatted: text[255];
        ShowAdditionalAddresses: Boolean;
    ...
    protected var
        ...
        FooterText: Label '** The banking instructions provided on this invoice will not be changed by Silver Bay Seafoods LLC. If you receive a request to change these banking instructions, please immediately contact your Silver Bay Seafoods sales representative by phone.**';
        FooterText2: Label 'Send remittance details to receivables@silverbayseafoods.com.  Thank you for your business.';
